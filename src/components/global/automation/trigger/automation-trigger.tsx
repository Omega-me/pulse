  ...
  if (automation?.data && automation?.data?.triggers.length > 0) {
    return (
      <div className="flex flex-col gap-y-6 items-center">
        <ActiveTrigger type={automation.data.triggers[0].type} keywords={automation.data.keywords} />
        {automation.data.triggers.length > 1 && (
          <>
            <div className="relative w-6/12">
              <p className="absolute transform bg-[#1d1d1d] px-2 -translate-y-1/2 top-1/2 -translate-x-1/2 left-1/2">or</p>
              <Separator orientation="horizontal" className="border-muted border-[1px]" />
            </div>
            <ActiveTrigger type={automation.data.triggers[1].type} keywords={automation.data.keywords} />
          </>
        )}
        ...
        {AUTOMATION_TRIGGERS.map((trigger) => (
          <div
            ...
            className={cn(
              ...
              !types?.find((t) => t === trigger.type) ? 'bg-muted' : 'bg-gradient-to-br from-[#3352cc] font-medium  to-[#1c2d70]'
            )}
          >
