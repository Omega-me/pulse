...
const Items = () => {
  ...
  return SIDEBAR_MENU.map((item) => {
    ...
    return (
      <div
        key={item.id}
        onClick={() => handleGoToRoute(`/dashboard/${item.label === 'home' ? '' : item.label}`)}
        className={cn(
          'cursor-pointer capitalize flex mb-2 gap-x-2 rounded-full p-3 transition-colors duration-300 ease-in-out',
